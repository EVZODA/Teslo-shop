...
export const CartList: FC<Props> = ({ editable=false }) => {
    ...
    const {cart,updateProductToCart, removeProductInCart} = useContext(CartContext)
                ...
                cart.map(product => (
                    <Grid container spacing={2} sx={{ mb: 1 }} key={product.slug + product.size}>
                        ...
                        <Grid item xs={2} display="flex" alignItems="center" flexDirection="column">
                            ...
                            {
                                editable && (<Button variant='text' color='secondary' onClick={()=>{removeProductInCart(product)}}>
                                    Remover
                                </Button>)
                            }
